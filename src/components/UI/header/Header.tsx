...
const Header: React.FC<{ handleProfileClick: () => void, isProfileActive: boolean, isSelectedGamePage: boolean }> = ({ handleProfileClick, isProfileActive, isSelectedGamePage }) => {
    ...
    const isHeaderGray = location.pathname.includes('referral')
    ...
    useEffect(() => {
        ...
        if (userId) {
            ...
            const referralLink = `t.me/adsawskdkweidskdk_bot/app?startapp=${userId}`
    ...
    return (
        isHeaderVisible && (
            <header className={`flex align__center justify__space__between ${styles.header} ${isProfileActive ? styles.profileActive : ''} ${isSelectedGamePage ? styles.absolute : ''}`}>
                <div className={`flex align_center ${styles.aboutUserContainer}`}>
                    ...
                    <div className={`flex column ${styles.userTextContainer}`}>
                        <span className={styles.username} onClick={handleProfileClick}>{userData ? userData.username : 'Загрузка..'}</span>
                        <TextWithCopy className='headerWalletContainer' id={styles.userWallet} text={referral.cryptedLink} linkValue={referral.uncryptedLink} isHeaderGray={isHeaderGray} />
                    </div>
                </div>

                <div className={`flex align__center ${styles.headerButtonsContainer}`}>
                    <a href='/notifications' type='button' className={`flex align__center justify__center headerNotificationButton ${(isHeaderGray || isSelectedGamePage) ? "lightButton" : ''} ${styles.headerButton}`}>
                    ...
                    <a href='/settings' type='button' className={`flex align__center justify__center headerSettingsButton ${(isHeaderGray || isSelectedGamePage) ? "lightButton" : ''} ${styles.headerButton}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">

...
import awp from '../../../static/img/awp.png'
import styles from './SelectedGame.module.scss'
import getImage from '../../../assets/getImage'
import { IGameData } from '../../../interfaces/IGameData'
import { useParams } from 'react-router-dom'
import { useGetGameActivityQuery, useGetUserQuery } from '../../../redux/services/userService'

const SelectedGame: React.FC = () => {
    const { data: userData, refetch: refetchUserData } = useGetUserQuery()
    const { data: gameActivityData, refetch: refetchGameActivity } = useGetGameActivityQuery()
    const [selectedGame, setSelectedGame] = useState<IGameData | null>(null)
    const [gamesData, setGamesData] = useState<IGameData[]>([])
    const [isLinkClicked, setLinkClicked] = useState<boolean>(false)

    const { id } = useParams()

    function getMonetsRecord(record?: number): string {
        const clickerRecordValue = record

        if (!clickerRecordValue || clickerRecordValue === 0) {
            return 'Отсутствует'
        }

        const getCoinWord = (count: number): string => {
        ...
        return `${clickerRecordValue} ${getCoinWord(clickerRecordValue)}`
    }

    const getDaysDeclension = (days: number): string => {
    ...
    const getDaysSince = (lastActivity: string | null): string => {
        if (!lastActivity) return 'Ожидание'
        ...
        try {
            ...
            return diffDays === 0 ? 'Сегодня' : getDaysDeclension(diffDays)
        } catch (error) {
            return 'Ожидание'
        }
    }
    ...
    useEffect(() => {
        if (!userData || !userData.balance || typeof userData.balance.tickets === 'undefined' || !gameActivityData) {
            refetchUserData()
            refetchGameActivity()
            return
        }
        ...
        setGamesData([
            {
                id: 1,
                href: 'guns',
                preview: 'twoElems.png',
                title: 'Соединение оружия',
                description: 'Объединяйте оружие, чтобы повысить его уровень. Соединяйте два одинаковых скина для получения более редкого оружия. На финальном этапе продайте скин за валюту или сохраните для вывода в Steam.',
                lastActivity: getDaysSince(gameActivityData.craft.last_activity),
                currentTickets: userData.balance.tickets,
                totalTickets: 3,
                record: gameActivityData?.craft.title || 'Отсутствует',
                recordPreview: gameActivityData?.craft.img_url || awp,
                recordBackground: '#2148E3',
                recordBorder: 'rgba(255, 255, 255, 0.5)',
                textColor: 'linear-gradient(0deg, #274784, #274784)',
                svgColor: 'rgba(39, 71, 132, 1)',
                width: '22px',
                height: '22px',
                backgroundColor: `linear-gradient(128.05deg, #590313 3.45%, #CE1133 45.26%, #DD1D40 88.77%)`,
                howPlay: [
                    "Перетаскивайте оружие в сетку.",
                    "Два одинаковых скина объединяются в один более высокого уровня.",
                    "Оружие последнего уровня анимируется, сигнализируя о готовности.",
                    `Выберите: <br/> <li class="circle">Продать скин за токены.</li> <li class="circle">Сохранить для вывода в Steam.</li>`,
                ],
            },
            {
                id: 2,
                href: 'gameTap',
                preview: 'money.png',
                title: 'Тапалка на время',
                description: 'Кликайте по появляющимся объектам в течение ограниченного времени, чтобы заработать очки. Чем быстрее и точнее клики, тем выше награда.',
                lastActivity: getDaysSince(gameActivityData?.clicker.last_activity),
                currentTickets: userData.balance.tickets,
                totalTickets: 3,
                record: gameActivityData?.clicker?.record ? getMonetsRecord(gameActivityData.clicker.record) : 'Отсутствует',
                recordBackground: 'rgba(255, 255, 255, 0.15)',
                recordPreview: 'transparentFire.png',
                textColor: 'linear-gradient(90deg, #B48241 0%, #804E3F 100%)',
                svgColor: 'rgba(136, 67, 46, 1)',
                width: '24px',
                height: '24px',
                backgroundColor: `linear-gradient(128.05deg, #C16E26 3.45%, #E3721F 45.26%, #F1FBD3 88.77%)`,
                howPlay: [
                    "Кликайте по объектам на экране.",
                    "Зарабатывайте очки за каждый успешный клик.",
                    "Достигайте максимального результата за отведенное время.",
                    `Выберите: <br/> <li class="circle">Обменять очки на токены.</li> <li class="circle">Сохранить для вывода в Steam.</li>`,
                ],
            },
            {
                id: 3,
                href: 'gameMoney',
                preview: 'handWithMoney.png',
                title: 'Ловля монет',
                description: 'Ловите падающие монеты, избегая препятствий. Чем больше монет собрано, тем выше награда.',
                lastActivity: getDaysSince(gameActivityData?.catching.last_activity),
                currentTickets: userData.balance.tickets,
                totalTickets: 3,
                record: gameActivityData?.catching?.record ? getMonetsRecord(gameActivityData.catching.record) : 'Отсутствует',
                recordBackground: 'rgba(255, 255, 255, 0.15)',
                recordPreview: 'transparentFire.png',
                textColor: 'linear-gradient(90deg, #4097C0 0%, #3D3574 100%)',
                svgColor: 'rgba(60, 62, 148, 1)',
                width: '24px',
                height: '24px',
                backgroundColor: `linear-gradient(128.05deg, #1732A9 3.45%, #2142D1 45.26%, #5874EE 88.77%)`,
                howPlay: [
                    "Ловите падающие монеты на экране.",
                    "Избегайте препятствий, чтобы не потерять очки.",
                    "Соберите максимальное количество монет за раунд.",
                    `Выберите: <br/> <li class="circle">Обменять монеты на токены.</li> <li class="circle">Сохранить для вывода в Steam.</li>`,
                ],
            },
        ])
    }, [userData, gameActivityData])

    useEffect(() => {
        if (!gamesData || !id || isNaN(parseInt(id))) return

        const parsedId = parseInt(id)
        const selected = gamesData.find((game) => game.id === parsedId)
    ...
    return (
        <div id={styles.selectedGamePopupOverlay}>
            {
                selectedGame && (
                    <div className={`flex column ${styles.selectedGamePopup}`}>
                        ...
                        <div className={`flex align__center justify__space__between ${styles.selectedGamePopupHeading}`}>
                            <div className={`flex align__center ${styles.userRecordCard}`}>
                                <div className={`flex align__center justify__center ${styles.previewRecord}`} style={{ 'background': selectedGame.recordBackground }}>
                                    <img src={selectedGame.recordPreview} alt='Рекорд' />
                                </div>
                                <div className={`flex column ${styles.selectedGameCardTextContainer}`}>
                                    ...
                                    <span className={styles.selectedGameHeadingCardDescription}>
                                        {selectedGame.record.replace(/_/g, ' ')}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className={`flex column ${styles.selectedGamePopupContent}`}>
                            <div className={`flex align__center justify__space__between ${styles.selectedGameUserResults}`}>
                                ...
                                <div className={`flex align__center ${styles.selectedGameResultsCard}`}>
                                    <div className={`flex align__center justify__center ${styles.recordPreview}`}>
                                        <img src={selectedGame.recordPreview} alt={selectedGame.title} />
                                    </div>

                                    <div className={`flex column ${styles.selectedGameResultsCardTextContainer}`}>
                                        <span className={styles.selectedGameResultsCardTitle}>Лучший резуль.</span>
                                        <span className={styles.selectedGameResultsCardDescription}>{selectedGame.record.replace(/_/g, ' ')}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <a href={`/${selectedGame.href}`} className={`flex align__center justify__space__between ${styles.startGameButton} ${isLinkClicked ? styles.inactive : ''}`} onClick={handleStartGame}>

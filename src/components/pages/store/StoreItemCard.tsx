...
const StoreItemCard: React.FC<{ itemCard: ICaseResponse, clickHandler: (itemCard: ICaseResponse) => void }> = ({ itemCard, clickHandler }) => {
    ...
    useEffect(() => {
        const fetchColors = async () => {
            try {
                const imageUrl = itemCard.img_url
    ...
    return (
        <div className={`flex align__center justify__space__between ${styles.itemCard}`}>
            <div className={`flex align__center ${styles.aboutItemCard}`}>
                <ImagePreview image={itemCard.img_url} />

                <div className={`flex column ${styles.aboutItemTextContainer}`}>
                    <span className={styles.itemTitle}>{itemCard.title}</span>
                    <span className={`text__center ${styles.itemRarity}`} style={{ backgroundColor: itemCard.rarityBackground, boxShadow: itemCard.rarityShadow ? itemCard.rarityShadow : `0px 0px 13.4px 0px ${brightestColor}, 0px 4px 4.8px 0px ${dominantColor} inset` }}>{itemCard.type}</span>
                </div>
            </div>
            ...
            <button type='button' className={`flex align__center justify__space__between ${styles.purchaseItemButton}`} onClick={() => clickHandler(itemCard)}>
                ...
                <div className={`flex align__end ${styles.itemPriceContainer}`}>
                    <span className={`flex align__center ${styles.itemPrice}`}>{formatPriceWithSpans(itemCard.donation_coins_price ? itemCard.donation_coins_price : itemCard.free_coins_price)}</span>
                    {
                        itemCard.donation_coins_price ? <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                        ...
                        </svg> : <svg xmlns="http://www.w3.org/2000/svg" width="12" height="16" viewBox="0 0 12 16" fill="none" style={{ width: '12px', height: '12px' }}>
